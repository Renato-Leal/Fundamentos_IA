...
class OthelloBoard(Board):
    ...
    def contar_fichas(self):
        ...
        print(f"Fichas B: {B_ficha}")
        print(f"Fichas A: {A_ficha}")

        if B_ficha > A_ficha:
            return f"¡Gana el jugador B!"

        elif A_ficha > B_ficha:
            return f"¡Gana el jugador A!"
    ...
    def alfaBeta(self, leTocaAlAgente, alfa, beta):
        global cont
        cont += 1
        ...
        if self.is_game_over(): 
            ...
            if ganador == 'B':           # humano gana -> -1
                return -1
            elif ganador == 'A':        # agente gana -> 1
                return 1
            elif self.is_full(): # empate -> 0
                return 0
            ...
            if leTocaAlAgente:       
                ...
                for i in range(3):
                    for j in range(3):
                        if self[i][j] == '.':
                            self[i][j] = 'A'                             #prueba la jugada i,j
                            puntaje = self.alfaBeta(self, False, alfa, beta)   #Regresa de la recursión
                            self[i][j] = '.'                             #borra la jugada i,j
                ...
                for i in range(3):
                    for j in range(3):
                        if self[i][j] == '.':
                            self[i][j] = 'B'
                            puntaje = self.alfaBeta(self, True, alfa, beta)
                            self[i][j] = '.'
                            mejorPuntaje = min(puntaje, mejorPuntaje) #Regresa de la recursión
